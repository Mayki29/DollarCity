...
<div class="container">
    <div class="table table-responsive">
        ...
        ?>
                ...
                <tr class="vertical-align-middle">
                    <td class="text-center"><img class="img img-fluid" src="<?php 
                        $inicial = substr($product['URLImagen'], 0, 4);
                        if($inicial == 'http'){
                            echo $product['URLImagen'];
                        }else{
                            echo $url_host . $product['URLImagen'];
                        }?>" alt=""></td>
                    <td><?=$product["Nombre"]?></td>
                    <td>S/. <?=$precio?></td>
                    <td style="width:100px"><input class="form-control" name="cantidad" type="number" min="1"
                            value="<?=$product["Cantidad"]?>" max="10"
                            onchange="actualizarCantidad(this.value,<?=$product['ProductoID']?>)">
                    </td>
                    <td>S/. <?=$subTotal?></td>
    ...
    <div class="row justify-content-end my-3">
        <div class="col-4">
            <a href="http://localhost/DollarCity/home/pago" class="btn btn-primary form-control"
                <?=$total <= 0 ? 'style="pointer-events: none; cursor: default;"' : '' ?>>Realizar Pago</a>
        </div>
    </div>
</div>
